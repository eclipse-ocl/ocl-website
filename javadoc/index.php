<?php 
	...
	{ 
		...
		$jdPWD = "/downloads/download.php?file=/modeling/mdt/javadoc/";
	}
	else
	{
...
<ul>
';
		sort($subprojs); reset($subprojs);
		foreach ($subprojs as $subproj) {
			if (in_array($subproj,$projects)) 
			{
				$trans = array_flip($projects);
				print '<li><b> '.$trans[$subproj].'</b>'."\n";
				$vers = loadDirSimple("$PWD/".$subproj,"","d"); rsort($vers); reset($vers);
				foreach ($vers as $ver) {
					if (preg_match("/[^0-9.]/",$ver)>0) { 
						$vers2 = loadDirSimple("$PWD/".$subproj."/".$ver,"","d"); rsort($vers2); reset($vers2);
						if (sizeof($vers2)>0)
						{
							print "<ul>\n";
						}
						foreach ($vers2 as $ver2) {
							print '<li><a href="'.$jdPWD.$subproj.'/'.$ver.'/'.$ver2.'/">'.$ver.' '.$ver2.'</a></li>'."\n";
						}
						if (sizeof($vers2)>0)
						{
							print "</ul>\n";
						...
						print '<ul><li><a href="'.$jdPWD.$subproj.'/'.$ver.'/">'.$subproj.' '.$ver.'</a></li></ul>'."\n";
					}
				}
				print '</li>'."\n";
			}
